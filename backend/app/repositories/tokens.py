...
def create_access_token(data: dict, expires_delta: timedelta | None = None) -> str:
    if not settings.access_token_secret:
        raise RuntimeError("ACCESS_TOKEN_SECRET no está configurado")
    ...
    return jwt.encode(
        claims, settings.access_token_secret, algorithm=settings.algorithm
    )
...
async def save_token(user_id: str | ObjectId, username: str, token: dict) -> dict:
    ...
    await run_collection(
        db_token, "replace_one", {"_id": object_id}, document, upsert=True
    )
    return {"_id": str(object_id), "username": username, **token}
...
async def get_token(user_id: str, refresh_token: str) -> dict | None:
    return await run_collection(
        db_token, "find_one", {"_id": ObjectId(user_id), "refresh_token": refresh_token}
    )
...
async def verify_refresh_token(refresh_token: str) -> dict:
    try:
        return jwt.decode(
            refresh_token, settings.access_token_secret, algorithms=[settings.algorithm]
        )
...
async def get_user_current(token: Annotated[str, Depends(oauth2_scheme)]) -> UserInDB:
    try:
        ...
        raise credentials_error() from error
    if not username or not payload.get("jti") or not (user := await get_user(username)):
        raise credentials_error()
    session = await run_collection(db_token, "find_one", {"_id": ObjectId(user.id)})
...
async def get_user_active_current(
    user: UserInDB = Depends(get_user_current),
) -> UserInDB:
    if not user.active:
...
async def verify_token(token: str) -> dict[str, bool]:
    try:
        ...
        if not user or not user.active or not payload.get("jti"):
            return {"valid": False}
        session = await run_collection(db_token, "find_one", {"_id": ObjectId(user.id)})
        token_hash = hashlib.sha256(token.encode()).hexdigest()
        return {
            "valid": bool(
                session
                and secrets.compare_digest(session.get("token_hash", ""), token_hash)
            )
        }
...
async def drop_tokenBD(token_id: str) -> dict[str, bool]:
    try:
        result = await run_collection(
            db_token, "delete_one", {"_id": ObjectId(token_id)}
        )
    except InvalidId:
        return {"valid": False}
