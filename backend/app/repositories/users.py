...
password_context = CryptContext(schemes=["pbkdf2_sha256"], deprecated="auto", pbkdf2_sha256__default_rounds=50000)
...
async def get_one_user(client: CreateUser | UpdateUser) -> dict | None:
    return await run_collection(db_user, "find_one", {"username": client.username}) if client.username else None


async def get_all_users() -> list[User]:
    return [User(**document) for document in await find_documents(db_user, {})]
...
async def update_user(user_id: str, data: UpdateUser) -> dict | None:
    ...
    result = await run_collection(db_user, "update_one", {"_id": ObjectId(user_id)}, {"$set": changes})
    return await run_collection(db_user, "find_one", {"_id": ObjectId(user_id)}) if result.matched_count else None
