...
app_prices = APIRouter(tags=["market-data"])
...
async def get_market_data(payload: MarketData) -> list[dict]:
    try:
        ...
        raise HTTPException(422, "No se pudieron procesar los datos para el periodo indicado") from error
    except Exception as error:
        logger.exception("Error inesperado consultando datos de mercado")
        raise HTTPException(503, "El servicio de mercado no está disponible temporalmente") from error
