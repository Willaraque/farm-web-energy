...
export default function ProductFormPage() {
  ...
  useEffect(() => {
    ...
    fetchTask(id)
      ...
      .catch((requestError) => {
        setError(getApiError(requestError, "No se pudo cargar el producto."));
        setStatus("error");
      });
  }, [id]);
  const update = (event) =>
  ...
  const remove = async () => {
    if (
      !window.confirm(
        `¿Eliminar “${form.name}”? Esta acción no se puede deshacer.`,
      )
    )
      return;
    setStatus("saving");
          ...
          <form onSubmit={submit}>
            ...
            <label className="field">
              ...
              <textarea
                ...
                onChange={update}
                placeholder="Información útil para identificar el producto"
              />
            </label>
            {error && (
